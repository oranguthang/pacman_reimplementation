...
void actors_update_positions(void) {
    ...
    for (i = 0; i < ACTOR_COUNT; ++i) {
        ...
        unsigned char pi = vi;
        unsigned char x_lo = (unsigned char)(spr_pos[(unsigned char)(pi + 1)] + obj_vel[(unsigned char)(vi + 1)]);
        unsigned char y_lo = (unsigned char)(spr_pos[(unsigned char)(pi + 3)] + obj_vel[(unsigned char)(vi + 3)]);
...
void actors_build_oam(void) {
    ...
    for (g = 0; g < SPR_GROUP_COUNT; ++g) {
        ...
        for (q = 0; q < SPR_QUAD_COUNT; ++q) {
            ...
            oam_buf[out] = (y == 0) ? 0xFF : (unsigned char)(y + k_oam_quad_offsets[o]);
            ...
            oam_buf[(unsigned char)(out + 3)] = (x == 0) ? 0xFF : (unsigned char)(x + k_oam_quad_offsets[(unsigned char)(o + 1)]);
        }
    }
}
...
void actors_rotate_runners(void) {
    ...
    for (i = RUNNER_FIRST; i < RUNNER_LAST; ++i) {
        unsigned char src = (unsigned char)((i + 1) << 2);
        unsigned char dst = (unsigned char)(i << 2);
    ...
    {
        unsigned char dst = (unsigned char)(RUNNER_LAST << 2);
